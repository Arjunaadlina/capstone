...
const Compare = () => {
  ...
  const [selectedCountry1, setSelectedCountry1] = useState(
    JSON.parse(localStorage.getItem("selectedCountry1")) || null
  );
  const [selectedCountry2, setSelectedCountry2] = useState(
    JSON.parse(localStorage.getItem("selectedCountry2")) || null
  );
  ...
  useEffect(() => {
    if (countries.length === 0) {
      ...
      const timer = setTimeout(() => {
        if (countries.length === 0) {
          window.alert(
            "Failed to fetch valid country data. Please refresh the page."
          );
  ...
  useEffect(() => {
    localStorage.setItem(
      "selectedCountry1",
      JSON.stringify(selectedCountry1)
    );
  }, [selectedCountry1]);
  ...
  useEffect(() => {
    localStorage.setItem(
      "selectedCountry2",
      JSON.stringify(selectedCountry2)
    );
  }, [selectedCountry2]);
